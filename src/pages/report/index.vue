  ...
  <view class="flex flex-col px-6">
    <view class="fixed top-0 left-0 w-full box-border px-6 pb-2 bg-white z-10">
      <view class="flex items-center bg-white border-2 border-solid border-gray-900 text-gray-900 text-xl font-bold rounded-2xl overflow-hidden shadow-gray">
        <view
          class="w-1_2 text-center rounded-xl px-3"
          :class="{'bg-yellow-400': !isCollect}"
          @tap="isCollect = false"
        >
          学习报告
        </view>
    ...
    <view class="mt-14">
      ...
      >
        <view class="mb-8">
          <view class="flex flex-col px-6 py-4 bg-white border-2 border-solid border-gray-900 text-gray-900 text-lg font-bold rounded-2xl shadow-gray">
            <view class="mb-2">
              累计使用 <text class="px-1 text-3xl text-red-500">
                {{ cumulativeTimes }}
              </text> 天
            </view>
            <view>
              今日学习 <text class="px-1 text-yellow-400">
                {{ duration+pastMinute }}
              </text> 分钟
            </view>
          </view>
      ...
      >
        <view
          v-if="collectCards.length > 0"
          class="flex flex-wrap -mx-2"
        >
          <view
            v-for="(item, index) in collectCards"
            :key="index"
            class="w-1_2 px-2 box-border mb-4"
            @tap="handleTo(index)"
          >
            <collect-item
        ...
        <view
          v-else
          class="py-4 text-center text-gray-700"
        >
          还没有收藏卡片哦~
        </view>
      </view>
